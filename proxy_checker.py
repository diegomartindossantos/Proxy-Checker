...
#CARGANDO LOS PROXIES DEL ARCHIVO EN UNA LISTA
...
def conexion(proxie):
    ...
    try:
        ...
        proxie_sin_puerto = proxie.split(":")[0]
        if ip_post_proxy == proxie_sin_puerto:
            print (f"[OK]   -   Esperado: {proxie_sin_puerto}   Recibido: {ip_post_proxy}")
            lista_proxies.append(ip_post_proxy)
        else:
            print (f" [BAD]    -   Esperado: {proxie_sin_puerto}   Recibido: {ip_post_proxy}")
    except requests.ConnectTimeout:
        print (f"[ERROR]   -   ({proxie}) ConnectTimeout")
        lista_proxies.pop
    except requests.exceptions.ProxyError:
        print (f"[ERROR]   -   ({proxie}) ProxyError")
    except requests.ConnectionError:
        print (f"[ERROR]   -   ({proxie}) ConnectionError")




tmout = int(input("TIMEOUT>> "))
print (f"[#] Nº Proxies: {len(lista_proxies)}")
opcion = input(f"¿Desea comprobar los {len(lista_proxies)} proxies? Y/N >>")
if opcion == "y" or opcion == "Y":
    for proxie in (lista_proxies):
        conexion(proxie)
